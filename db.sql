...
CREATE TABLE `tb_blacklist` (
  ...
  `status` int(4) DEFAULT 0 NOT NULL COMMENT '0:正常，1:灰名单, 2:黑名单',
  `reason` varchar(256) DEFAULT NULL COMMENT '非正常的原因',
  `platform` varchar(256) DEFAULT NULL COMMENT '第三方平台标识',
  `origin_message` varchar(2048) DEFAULT NULL COMMENT '三方服务返回的原始信息',
  `expire_time` datetime NOT NULL DEFAULT '2199-01-01 00:00:00' COMMENT '操作截止时间',
  `create_time` datetime NOT NULL COMMENT '记录创建时间',
  PRIMARY KEY (`id`),
  KEY `cid_no` (`cid_no`),
