...
CREATE TABLE if NOT EXISTS `tb_apply_info` (
  ...
  `comment` varchar(256) DEFAULT NULL COMMENT '备注',
  `order_number` varchar(256) DEFAULT NULL COMMENT '自动放款订单号，唯一',
...
CREATE TABLE `tb_user_basic_info` (
  ...
  `info_id` varchar(32) NOT NULL COMMENT '资料标识id',
  `user_id` int(10) NOT NULL COMMENT '关联用户id',
  `medical_insurance_pic` varchar(128) not null default '' comment '医保证件照片',
  `refund_proof_pic` varchar(128) not null default '' comment '还款证明照片',
  `driving_license_pic` varchar(128) not null default '' comment '驾照照片',
